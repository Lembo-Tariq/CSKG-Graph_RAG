"""
evaluate_triple_full.py
=======================
Evaluates Triple RAG (CSKG facts) on all 734 papers
at multiple K values: K = 1, 3, 5, 10, 20
Uses the same queries as evaluate_baseline_full.py
...
# ─────────────────────────────────────────────
# Load Triple RAG Chroma DB
# ─────────────────────────────────────────────
# Key difference from baseline — we load the
# CSKG triples DB not the abstracts DB

db_dir = os.path.join(current_dir, "..", "Normal_RAG", "db", "chroma_db")
print("Loading Triple RAG Chroma DB...")
...
def get_rank(retrieved_chunks, correct_paper_id):
    """
    Searches for correct paper ID in retrieved chunks.
    Triple DB facts end with (paper: W1234567)
    so we check if the W ID appears in the chunk.
    """
...
print("Running Triple RAG evaluation for K values: " + str(K_VALUES))
...
for i, sample in enumerate(eval_dataset):
    ...
    # Get rank in full retrieved list
...
print("\n" + "=" * 60)
print("TRIPLE RAG RESULTS - FULL EVALUATION (734 papers)")
print("=" * 60)
...
output = {
    "evaluation_type": "triple-rag-full-734-papers",
...
json_file = os.path.join(current_dir, "triple_eval_full_results.json")
with open(json_file, "w", encoding="utf-8") as f:
    json.dump(output, f, indent=2, ensure_ascii=False)

csv_file = os.path.join(current_dir, "triple_eval_full_results.csv")
...
print("\nResults saved to triple_eval_full_results.json and .csv")